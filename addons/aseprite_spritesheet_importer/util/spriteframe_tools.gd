	...
	for i in range(self._frame_regions.size()):
		for layer_name: String in self._frame_regions[i]:
			for slice_name: String in self._slices:
				var spriteframes_name: String = "%s_%s_%s" % [self.spritesheet_name, slice_name, layer_name]
...
func _flatten_slice_regions() -> void:
	...
	for i in range(self._frame_regions.size()):
		for slice_name: String in self._slices:
			var spriteframes_name: String = "%s_%s" % [self.spritesheet_name, slice_name]
...
func _flatten_frame_regions_split() -> void:
	...
	for i in range(self._frame_regions.size()):
		for layer_name: String in self._frame_regions[i]:
			var spriteframes_name: String = "%s_%s" % [self.spritesheet_name, layer_name]
...
func _flatten_frame_regions() -> void:
	...
	self._named_spriteframes_atlas_names = {}
	for i in range(self._frame_regions.size()):
		var spriteframes_name: String = self.spritesheet_name
...
func _save_spriteframes() -> Error:
	for spriteframes_name: String in self._named_spriteframes:
		...
		EditorInterface.get_resource_filesystem().update_file(sprite_frames_path)
...
func _add_animations() -> void:
	...
	for tag: Dictionary in self.spritesheet_data.meta.frameTags:
		...
		for spriteframes_name: String in self._named_spriteframes:
			...
			# Animation direction
			var frame_range: Array = range(tag.from, tag.to+1) # forwards
			...
			if !self.ignore_framerate:
				...
				var ms_per_frame: float = float(self._frame_durations[tag.from])
				var fps: float = 1.0 / (ms_per_frame / 1000.0)
				spriteframes.set_animation_speed(anim_name, roundf(fps))

				for i: int in frame_range:
					var frame_duration: float = self._frame_durations[i] / ms_per_frame
					var atlas_names: Array = _named_spriteframes_atlas_names[spriteframes_name]
